...
! function ()
{
	...
	var camera = {
		...
		setTarget: function (elem) {
			...
			// set target
			if (this.targeted) {
				this.targeted.style.outline = "rgba(0,0,0,0) solid 10px";
			}
			if (this.targeted != elem) {
				...
				elem.style.outline = "rgba(255,255,255,1) solid 0.5rem";
				this.targeted = elem;
			} else {
				this.z.setTarget(0);
				if (elem.id == "7") {
		...
		move: function () {
			...
			if (pointer.touchMode) {
				...
				var x = -pointer.dy * 0.006;
				var y = pointer.dx * 0.006;
				pointer.center(0.98);	
			} else {
			...
			// cube transformation
			cube.style[transform] = "translateZ("+ (this.z.value) + "rem) rotateX(" + (this.rx.value + this.globalRX) + "rad) rotateY(" + (this.ry.value + this.globalRY) + "rad)";
		}
	}
	// main loop
	var run = function () {
